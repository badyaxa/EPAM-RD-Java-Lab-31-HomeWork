...
import com.epam.spring.homework2.beans.BeanClassB;
import com.epam.spring.homework2.beans.BeanClassC;
import com.epam.spring.homework2.beans.BeanClassD;
...
public class BeansConfigFromProperties {
    ...
    @Bean(value = "beanB", initMethod = "customInitMethodB", destroyMethod = "customDestroyMethodB")
    @DependsOn(value = "beanD")
    public BeanClassB getBeanB() {
        return new BeanClassB(nameFieldB, valueFieldB);
    }

    @Bean(value = "beanC", initMethod = "customInitMethodC", destroyMethod = "customDestroyMethodC")
    @DependsOn(value = "beanB")
    public BeanClassC getBeanC() {
        return new BeanClassC(nameFieldC, valueFieldC);
    }

    @Bean(value = "beanD", initMethod = "customInitMethodD", destroyMethod = "customDestroyMethodD")
